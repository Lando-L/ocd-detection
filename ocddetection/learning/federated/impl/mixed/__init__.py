...
from ocddetection.learning.federated.impl.mixed import client, process
...
def __client_state_fn(
	...
	mixing_coefficients: List[tf.Tensor]
) -> client.State:
	return client.State(
...
def setup(
	...
	client_state_fn = partial(__client_state_fn, idx=-1, pos_weight=1.0, weights=weights, mixing_coefficients=mixing_coefficients)
	client_states = partial(__client_state_fn, weights=weights, mixing_coefficients=mixing_coefficients)

	iterator = process.iterator(coefficient_fn, model_fn, client_state_fn, server_optimizer_fn, client_optimizer_fn)
	validator = process.validator(coefficient_fn, model_fn, client_state_fn)
	evaluator = process.evaluator(coefficient_fn, eval_model_fn, client_state_fn)

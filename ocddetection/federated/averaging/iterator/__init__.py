...
from ocddetection.federated.averaging import client, server
...
def iterator(
    ...
    def next_tff(state, dataset):
        message = tff.federated_map(state_to_message_tf, state)
        broadcast = tff.federated_broadcast(message)

        outputs = tff.federated_map(update_client_tf, (dataset, broadcast))
